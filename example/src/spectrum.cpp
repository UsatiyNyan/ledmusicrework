...
gboolean UpdateSpectrum(gpointer data) {
    ...
    if (specmode == 3) { // waveform
        ...
        BASS_ChannelGetData(chan,
                            buf,
                            (ci.chans * SPECWIDTH * sizeof(float))
        ...
        for (c = 0; c < ci.chans; c++) {
            for (x = 0; x < SPECWIDTH; x++) {
                ...
                do { // draw line from previous sample...
                    ...
                    specbuf[y * SPECWIDTH + x] = palette[c & 1 ? 127
                                                               : 1]; // left=green, right=red (could add more colours to palette for more chans)
                } while (y != v);
            }
        }
    } else {
            ...
            for (x = 0; x < BANDS; x++) {
                ...
                while (--y >= 0)
                    for (y1 = 0; y1 < SPECWIDTH / BANDS - 2; y1++)
                        specbuf[(SPECHEIGHT - 1 - y) * SPECWIDTH + x * (SPECWIDTH / BANDS) + y1] =
                            palette[y + 1]; // draw bar
            }
        } else { // "3D"
